...
with graph.as_default():
	train_dataset = tf.placeholder(tf.float32, 
								   shape = (batch_size, im_size*im_size))
	...
	g1 = tf.Variable(tf.truncated_normal([1, 96]))
	v1 = tf.Variable(tf.truncated_normal([im_size*im_size, 96]))
	b1 = tf.Variable(tf.truncated_normal([96]))
	cur_layer = tf.matmul(cur_layer, tf.multiply(g1,v1/tf.norm(v1, axis = (0,1))))
